...
const validIpv4 = string().test("is Ipv4", (value, context) => {
    ...
    if (!ipV4Regex.test(value)) {
        return context.createError({
            message: `L'indirizzo ip del ${parent} non è nella forma corretta ${chalk.red(context.originalValue)}`,
        });
    }
    return true;
});
const validPort = string().test("is valid port", (value, context) => {
    ...
    if (!/^\d+$/.test(value)) {
        return context.createError({
            message: `La porta del ${parent} non ha la forma corretta ${chalk.red(value)}`,
        });
    }
    return true;
});
