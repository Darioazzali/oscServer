...
import { readConfigurationFromFile, } from "../run/readFromConfigFile.js";
...
describe("Tests", () => {
    ...
    it("Read from a config file", () => {
        const data = JSON.parse(fs.readFileSync(path.join(__dirname, "mockedConfig.json"), {
            encoding: "utf-8",
        }));
        expect(data).toEqual({
    ...
    describe("Validation with Yup", () => {
        ...
        it("throw 'la porta del server non ha la forma corretta' if a port number is in the wrong format", () => {
            ...
            expect(() => configSchema.validateSync(correctConfig)).toThrowError("La porta del server non ha la forma corretta");
        });
        it("throw 'manca la porta del server' if a port number is empty", () => {
            ...
            expect(() => configSchema.validateSync(correctConfig)).toThrowError("Non è stata specificata la porta del server");
        });
        it("throw 'l'indirizzo ip del oscTarget è mancante' if the oscTarget IPv4 is empty", () => {
            ...
            expect(() => configSchema.validateSync(correctConfig)).toThrowError(`L'indirizzo Ip del oscTarget è mancante`);
        });
        it("throw 'l'indirizzo ip del oscTarget non è nella forma corretta' if the oscTarget IPv4 is malformed", () => {
            ...
            expect(() => configSchema.validateSync(correctConfig)).toThrowError(`L'indirizzo ip del oscTarget non è nella forma corretta`);
        });
    });
});
