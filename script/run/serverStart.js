...
function readEnvFiles() {
    const frontedData = readEnvFile(path.join(__dirname, "..", "..", "frontend", ".env"));
    const backendData = readEnvFile(path.join(__dirname, "..", "..", "oscServer", ".env"));
    return {
        frontend: {
...
function printActualConfig(envConfig) {
    console.log(`${chalk.bgGreen("Frontend")}:\n Indirizzo Ip: ${envConfig.frontend.ipAddress}\n Porta: ${envConfig.frontend.port}`);
    console.log(`${chalk.bgGreen("Backend")}:\n Indirizzo Ip: ${getIpAddresses().ipAddress} (Locale)\n Porta: ${envConfig.backend.port}`);
    console.log(`${chalk.bgGreen("Osc Target")}:\n Indirizzo Ip: ${envConfig.oscTarget.ipAddress}\n Porta: ${envConfig.oscTarget.port}`);
}
//Verifica che i parametri inseriti permettano l'avvio del server
function verifyConfigVariables(envConfig) {
